...
- **The client directory (Network tab) is DB-backed.** `GET /v1/projects` reads the `directory_entries` table, not the legacy `PROJECTS` env var. On first startup the server performs a **one-time seed**: if the table is empty and `PROJECTS` is set, it inserts those entries as unowned rows (`project_id = NULL`), **preserving each entry's operator-set `official` flag and OAuth `client_id`**; thereafter the directory is DB-owned and `PROJECTS` edits are ignored. This means testbot (whose entry the deploy still writes into `PROJECTS`) keeps its Network-tab listing transitively via the seed, with no deploy-side changes. Manifest-driven entries added later via `/install-project` are always non-official. **Deferred:** reconciling the deploy-side `avalanche-install-project` CLI / `regenerate_projects` so the CLI drives manifests and the `PROJECTS`-env generation retires (`02-todos-deferred.md`).
