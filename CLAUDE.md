...
- **Design-check every user-visible frontend change.** Any new or changed UI (a view, card, button, chip, row, badge, empty state, etc. on iOS/Android/Desktop) must be styled deliberately, not left on placeholder defaults. Before considering it done: (a) use the app's **design tokens**, never ad-hoc or platform-system colors — iOS pulls from the semantic aliases in `mobile/ios/Actnet/Sources/Utils/AvalancheColors.swift` (`Color.avCard`, `avIncomingBubble`, `avMuted`, …), and Android/Desktop use their equivalents; a raw `Color(.secondarySystemBackground)`, hardcoded hex, or default gray is a red flag; (b) **match the nearest existing sibling component** (e.g. a new in-bubble card should mirror `LinkPreviewCard`'s background/corner-radius/border) so it reads as part of the same family; (c) confirm it adapts to **light and dark** (the semantic aliases do this for free — don't bypass them); (d) call out the styling choices in your summary so they're reviewable. When unsure how something should look, ask or propose options rather than shipping a placeholder. The cross-platform parity rule applies to visual style too, not just behavior.
